...
export const TopologyNode = ({ data }: { data: TopologyNodeData }) => {
  ...
  return (
    <div className={cn(
      'px-5 py-4 rounded-2xl border-2 shadow-xl min-w-[220px] max-w-[220px] bg-white dark:bg-zinc-900 transition-all',
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <h3 className="text-sm font-extrabold leading-tight mt-1 break-words">{resource.name}</h3>
        </div>
      </div>

      <div className="mt-3 pt-3 border-t border-current/10">
        <p className="text-[10px] font-medium leading-relaxed opacity-80 break-words">{resource.details}</p>
      </div>
